...
# ---------------- PRICE ----------------
def getPriceHistory(company):
    df = company.history(period='6mo')  # reduced from 12mo for performance
...
def getEarningsDate(company):
    try:
        ...
        return [
            d.tz_localize(None).strftime("%Y-%m-%d")
            for d in df.index
            if d.tz_localize(None) > now
        ]
    except:
        return []
...
def getCompanyNews(company):
    ...
    # LIMIT NEWS → VERY IMPORTANT FOR MEMORY
    for item in (company.news or [])[:3]:
        content = item.get("content", {})
        title = content.get("title", "")
        summary = content.get("summary", "")
        url = content.get("canonicalUrl", {}).get("url", "")
...
# ---------------- SAFE TEXT BUILDING ----------------
def buildNewsText(newsArticles):
    # DO NOT SCRAPE FULL ARTICLES (REMOVED ON PURPOSE)
    text = ""
...
# ---------------- MAIN ----------------
def getCompanyStockInfo(tickerSymbol):
    company = yf.Ticker(tickerSymbol.upper().strip())
    ...
    if not info:
        raise NameError("Invalid ticker or blocked request")

    basicInfo = extractBasicInfo(info)

    if not basicInfo.get("longName"):
        raise NameError("Ticker not found or delisted")
    ...
    # LIGHTWEIGHT TEXT ANALYSIS INPUT
    newsText = buildNewsText(newsArticles)

    # NLP (already optimized version you cleaned earlier)
    newsTextAnalysis = analyze.analyzedText(newsText)
